...
//   console.log('getOlivData::::', getOlivData)
...
export function HandleStep({
  ...
  const getOlivData =
    typeof window !== "undefined"
      ? JSON.parse(localStorage.getItem("olivData") || "{}")
      : {};

  // ✅ Auto-fill priority
  const initialHandle =
    data.handle ||
    getOlivData?.candidate?.username ||
  ...
  // Restore verified state when coming back
  useEffect(() => {
    if (data.handleVerified) {
      setIsAvailable(true);
      setButtonText("Continue →");
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await createAvatar({
        user_name: handleValue,
        email: getOlivData?.email || getOlivData?.candidate?.email,
        oliv_id: getOlivData?.id || getOlivData?.candidate?.id,
      });
      ...
      toast.error(
        resData?.errors?.user_name?.[0] ||
          resData?.errors?.oliv_id?.[0] ||
          resData?.message ||
          "This link is already taken"
      );
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <div
            ...
            onMouseLeave={() => setHovered(false)}
          >
            <div className="absolute -translate-y-1/2 left-3 top-1/2 text-muted-foreground font-medium">
              {url.host}/
            </div>
            ...
            <Input
              ...
              onChange={(value: string) => {
                ...
                setHandleValue(sanitized);
                setIsAvailable(null);
                setButtonText("Confirm");
                updateData({ handle: sanitized, handleVerified: false });
              }}
              placeholder="yourname"
              className="pr-10"
              style={{ paddingLeft: `${url.host.length * 8 + 28}px` }}
              required
            />
